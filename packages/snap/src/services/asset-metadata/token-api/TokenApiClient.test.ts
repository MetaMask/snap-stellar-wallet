...
describe('TokenApiClient', () => {
  ...
  describe('getTokensMetadata', () => {
    it('returns empty array when assetIds is empty', async () => {
      const client = new TokenApiClient(mockFetch, logger);
    ...
    it('wraps HTTP errors in TokenApiException', async () => {
      mockFetch.mockResolvedValueOnce(
    ...
    it('wraps fetch rejections in TokenApiException', async () => {
      mockFetch.mockRejectedValueOnce(new Error('network down'));

      const client = new TokenApiClient(mockFetch, logger);
      await expect(client.getTokensMetadata([classicAssetId])).rejects.toThrow(
        TokenApiException,
      );
    });
  });
